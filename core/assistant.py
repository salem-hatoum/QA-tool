...
assistant_voice_recognition_enabled = False  # Disable if you don't want to use voice recognition
...
def process_command(command):
    message_low = command.lower()
    if "open" in message_low[:4]:
        if len(command) < 18:
            print("Opening the program:", command)
            activate_windowt_title(command.strip("open "))
        else:
            assistant(command)
    elif "double click" in message_low[:12]:
        print("Double clicking on:", command)
        fast_act(single_step=command.strip("double "), double_click=True)
    elif any(keyword in message_low[:8] for keyword in ["click on", "click the", "click"]):
        print("Clicking on:", command)
        fast_act(single_step=command)
    elif "press" in message_low[:5]:
        print("Pressing:", command)
        perform_simulated_keypress(command.strip("press ").strip())
    elif any(keyword in message_low[:6] for keyword in ["type", "write", "bright", "great"]):
        new_message = command.replace("bright ", "").replace("write ", "").replace("type ", "").replace("great ", "")
        print("Typing:", new_message)
        write_action(goal=new_message, last_step="text_entry")
    elif any(keyword in message_low for keyword in ["reminder", "remind", "timer", "alarm"]):
        print("Reminder:", command)
        # You may want to call a function to handle reminders, timers, alarms, etc.
    elif "scroll" in message_low[:6]:
        print("Scrolling:", command)
        import pyautogui
...
def listen_thread():
    ...
    if not assistant_voice_recognition_enabled:
        # If voice recognition got disabled, wait for a bit before checking again
        message = read_text_file("./qa.txt")
        # Split the message by lines and process each line separately
        if message:
            for line in message.split('\n'):
                process_command(line)
        # You may want to consider using a condition variable instead for better efficiency
    while assistant_voice_recognition_enabled:  # Moved the condition here
        with sr.Microphone() as source:
            print("Listening...")
            try:
                ...
                if assistant_voice_recognition_enabled:
                    ...
                    if "okay computer" in message_low[:13] or assistant_name_handle.lower() in message_low[:11]:
...
def auto_prompt(message):
    ...
    # Extract the role name without the prefix and suffix
    role_name = role_function.replace("windows_assistant", "").replace("joyful_conversation", "").replace("-", "").strip("\\_")
    
    # Print the role name of the assistant
    print(f"Assistant: {role_name}")
    
    if "windows_assistant" in role_function or "windows\\_assistant" in role_function:
        # Queue the message for the assistant
        message_queue.put(role_name)
        
        # Start the assistant in a new thread
        assistant_thread = threading.Thread(target=run_assistant, args=(message,))
        assistant_thread.start()
    elif "joyful_conversation" in role_function or "joyful\\_conversation" in role_function:
        # Queue a joyful conversation message
        message_queue.put(f"{role_name} How can I help you?")
    else:
        print("NOT WORKING")
